...
const Profile = () => {
  ...
  const handleUpdate = async () => {
    // MongoDB Logic: এখানে patch/put রিকোয়েস্ট হবে
    setIsEditing(false);
  };

  return (
    <div className="min-h-screen bg-[#05070a] text-white py-10 md:py-20 px-4 sm:px-6 lg:px-8">
      {/* Ambient BG effects - Hidden on small screens to improve performance */}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
              ...
              <div className="relative shrink-0">
                <div className="w-32 h-32 md:w-44 md:h-44 rounded-3xl md:rounded-[2.5rem] bg-gradient-to-tr from-green-500 to-emerald-400 p-[2px]">
                  <img 
                    ...
                    className="w-full h-full rounded-[1.8rem] md:rounded-[2.4rem] object-cover bg-black"
                    alt="User"
                  />
                </div>
                {isEditing && (
              ...
              <div className="flex-grow text-center sm:text-left space-y-4">
                ...
                {isEditing ? (
                  <input 
                    ...
                    className="block w-full bg-white/5 border-b border-green-500 text-2xl md:text-4xl font-black italic uppercase tracking-tighter outline-none py-1 transition-all"
                  />
                ) : (
                  <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black italic uppercase tracking-tighter leading-tight">
          ...
          <motion.div 
            ...
            viewport={{ once: false }}
            className="md:col-span-4 bg-green-500 rounded-[2rem] md:rounded-[3rem] p-6 md:p-8 flex md:flex-col justify-between items-center md:items-start"
          >
            ...
            <div className="flex flex-col items-end md:items-start text-black">
              <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Control</p>
              <AnimatePresence mode="wait">
                {isEditing ? (
                  <div className="flex gap-2 mt-2">
                    <button onClick={handleUpdate} className="p-3 bg-black text-white rounded-xl hover:scale-105 active:scale-95 transition-all">
                      <Check size={20} />
                    </button>
                    <button onClick={() => setIsEditing(false)} className="p-3 bg-white/30 text-black rounded-xl">
                      <X size={20} />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 mt-1 py-2 px-4 bg-black/10 rounded-xl hover:bg-black/20 transition-all font-black text-xs uppercase italic tracking-tighter">
          ...
          {[
            ...
            <motion.div 
              ...
              className="md:col-span-4 bg-[#0d1117] border border-white/5 rounded-[2rem] p-6 md:p-8 group hover:border-green-500/20 transition-colors"
            >
        ...
        <motion.div 
          ...
          className="mt-8 flex flex-wrap justify-center gap-3 opacity-40 hover:opacity-100 transition-opacity"
        >
